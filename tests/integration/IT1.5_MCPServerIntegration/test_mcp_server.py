...
@pytest.mark.asyncio
async def test_mcp_server_integration_if_configured() -> None:
    base_url = os.environ.get("MCP_TEST_URL", "").strip()
    if base_url:
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            if request.url.path.endswith("/messages"):
                return httpx.Response(200, json={"result": {"tools": [{"name": "fallback"}]}})
