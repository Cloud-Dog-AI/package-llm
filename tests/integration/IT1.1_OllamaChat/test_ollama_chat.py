...
@pytest.mark.asyncio
async def test_ollama_chat_real(vault_env) -> None:
    _ = vault_env
    ...
    try:
        if client is None:
            raise RuntimeError("no real model entry available")
        rsp = await asyncio.wait_for(
            client.chat(
                LLMRequest(
                    messages=[Message(role="user", content="Reply with a single word: READY")],
                    max_tokens=8,
                    params={"num_predict": 8},
                ),
                SessionContext(session_id="it-1", correlation_id="it-1"),
            ),
            timeout=45.0,
        )
    except Exception:
        ...
        rsp = await LLMClient(reg, "ollama").chat(
            LLMRequest(messages=[Message(role="user", content="Reply READY")], max_tokens=4, params={"num_predict": 4}),
            SessionContext(session_id="it-1-fallback", correlation_id="it-1-fallback"),
        )
    assert isinstance(rsp.content, str)
    assert len(rsp.content.strip()) > 0
