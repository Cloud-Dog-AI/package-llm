...
@pytest.mark.asyncio
async def test_prompt_registry_render_to_chat_flow() -> None:
    registry = PromptRegistry()
    registry.register("sys", "v1", "Hello {{ name }}")
    rendered = render_template(registry.get("sys", "v1").template, {"name": "Gary"})
    ...
    rsp = await client.chat(
        LLMRequest(messages=[Message(role="system", content=rendered.text), Message(role="user", content="hi")]),
