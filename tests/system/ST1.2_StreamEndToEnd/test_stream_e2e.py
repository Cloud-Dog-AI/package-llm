...
@pytest.mark.asyncio
async def test_stream_end_to_end() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path.endswith("/api/chat"):
            return httpx.Response(200, json={"message": {"content": "streamed"}})
    ...
    assert [e.type.value for e in events] == ["response_started", "delta_text", "response_completed"]
