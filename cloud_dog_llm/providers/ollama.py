...
class OllamaAdapter(ProviderAdapter):
    ...
    async def invoke(self, request: LLMRequest) -> LLMResponse:
        ...
        # Thinking mode: default OFF to prevent qwen3 empty-response bug.
        # Set request.think=True to enable (e.g., for reasoning tasks).
        # think_budget limits thinking tokens (Ollama "think_budget" option).
        think = request.think if request.think is not None else False
        body = {
            "model": model,
            "messages": messages,
            "stream": False,
        ...
        if think and request.think_budget is not None:
            body["options"]["think_budget"] = int(request.think_budget)
        try:
